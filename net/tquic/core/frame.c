...
#define TQUIC_MAX_FRAME_SIZE		(16 * 1024)  /* 16 KB */
#define TQUIC_MAX_REASON_LEN		256
#define TQUIC_MAX_TOKEN_LEN		(8 * 1024)   /* 8 KB */
#define TQUIC_STATELESS_RESET_TOKEN_LEN	16

/*
 * ACK Range structure for ACK frames
 */
...
struct tquic_frame_padding {
	size_t length;  /* Number of padding bytes */
};
...
struct tquic_frame_ack {
	...
	/* ECN counts (only present if frame type is ACK_ECN) */
	bool has_ecn;
	u64 ect0_count;
	u64 ect1_count;
	u64 ecn_ce_count;
...
struct tquic_frame_max_streams {
	u64 max_streams;
	bool bidi;  /* true for bidirectional, false for unidirectional */
};
...
struct tquic_frame_connection_close {
	u64 error_code;
	u64 frame_type;  /* Only for transport close (0x1c) */
	u64 reason_len;
	const u8 *reason;
	bool app_close;  /* true if application close (0x1d) */
};
...
/**
 * tquic_varint_len - Get the encoded length of a varint value
 * @val: Value to encode
 *
 * Returns the number of bytes needed to encode the value, or 0 if
 * the value is too large.
 */
static inline size_t tquic_varint_len(u64 val)
{
	if (val <= 63)
		return 1;
	if (val <= 16383)
...
/**
 * tquic_varint_decode_len - Get length from first byte of encoded varint
 * @first_byte: First byte of the encoded varint
 *
 * Returns the total number of bytes in the encoded varint.
 */
static inline size_t tquic_varint_decode_len(u8 first_byte)
{
 ...
 */
static int tquic_varint_decode(const u8 *buf, size_t buf_len,
			       u64 *val, size_t *consumed)
{
	...
	len = tquic_varint_decode_len(buf[0]);
	if (buf_len < len)
		return -EINVAL;
...
/**
 * tquic_varint_encode - Encode a QUIC variable-length integer
 * @val: Value to encode
 ...
 */
static int tquic_varint_encode(u64 val, u8 *buf, size_t buf_len)
{
	size_t len = tquic_varint_len(val);
				 ...
				 size_t max_ranges)
{
	...
	ret = tquic_varint_decode(p, remaining, &frame->ack.largest_ack, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
	...
	/* ACK Delay */
	ret = tquic_varint_decode(p, remaining, &frame->ack.ack_delay, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
	...
	/* ACK Range Count */
	ret = tquic_varint_decode(p, remaining, &frame->ack.ack_range_count, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
	...
	/* First ACK Range */
	ret = tquic_varint_decode(p, remaining, &frame->ack.first_ack_range, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
	...
	for (i = 0; i < frame->ack.ack_range_count; i++) {
		/* Gap */
		ret = tquic_varint_decode(p, remaining, &ranges_buf[i].gap, &consumed);
		if (ret < 0)
			return ret;
		p += consumed;
		remaining -= consumed;
		...
		/* ACK Range Length */
		ret = tquic_varint_decode(p, remaining, &ranges_buf[i].ack_range_len, &consumed);
		if (ret < 0)
			return ret;
		p += consumed;
		remaining -= consumed;
	...
	if (has_ecn) {
		ret = tquic_varint_decode(p, remaining, &frame->ack.ect0_count, &consumed);
		if (ret < 0)
			return ret;
		p += consumed;
		remaining -= consumed;

		ret = tquic_varint_decode(p, remaining, &frame->ack.ect1_count, &consumed);
		if (ret < 0)
			return ret;
		p += consumed;
		remaining -= consumed;

		ret = tquic_varint_decode(p, remaining, &frame->ack.ecn_ce_count, &consumed);
		if (ret < 0)
			return ret;
		p += consumed;
		remaining -= consumed;
					  ...
					  struct tquic_frame *frame)
{
	...
	/* Stream ID */
	ret = tquic_varint_decode(p, remaining, &frame->reset_stream.stream_id, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
	...
	ret = tquic_varint_decode(p, remaining, &frame->reset_stream.app_error_code, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
	...
	/* Final Size */
	ret = tquic_varint_decode(p, remaining, &frame->reset_stream.final_size, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
					  ...
					  struct tquic_frame *frame)
{
	...
	/* Stream ID */
	ret = tquic_varint_decode(p, remaining, &frame->stop_sending.stream_id, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
	...
	/* Application Protocol Error Code */
	ret = tquic_varint_decode(p, remaining, &frame->stop_sending.app_error_code, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
				    ...
				    struct tquic_frame *frame)
{
	...
	/* Offset */
	ret = tquic_varint_decode(p, remaining, &frame->crypto.offset, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
	...
	/* Length */
	ret = tquic_varint_decode(p, remaining, &frame->crypto.length, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
				       ...
				       struct tquic_frame *frame)
{
	...
	/* Token Length */
	ret = tquic_varint_decode(p, remaining, &frame->new_token.token_len, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
				    ...
				    struct tquic_frame *frame)
{
	...
	/* Stream ID */
	ret = tquic_varint_decode(p, remaining, &frame->stream.stream_id, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
	...
	if (frame->stream.has_offset) {
		ret = tquic_varint_decode(p, remaining, &frame->stream.offset, &consumed);
		if (ret < 0)
			return ret;
		p += consumed;
		remaining -= consumed;
	...
	if (frame->stream.has_length) {
		ret = tquic_varint_decode(p, remaining, &frame->stream.length, &consumed);
		if (ret < 0)
			return ret;
		p += consumed;
		remaining -= consumed;
				      ...
				      struct tquic_frame *frame)
{
	...
	/* Maximum Data */
	ret = tquic_varint_decode(p, remaining, &frame->max_data.max_data, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
					     ...
					     struct tquic_frame *frame)
{
	...
	/* Stream ID */
	ret = tquic_varint_decode(p, remaining, &frame->max_stream_data.stream_id, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
	...
	/* Maximum Stream Data */
	ret = tquic_varint_decode(p, remaining, &frame->max_stream_data.max_stream_data, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
					 ...
					 struct tquic_frame *frame, bool bidi)
{
	...
	/* Maximum Streams */
	ret = tquic_varint_decode(p, remaining, &frame->max_streams.max_streams, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
					  ...
					  struct tquic_frame *frame)
{
	...
	ret = tquic_varint_decode(p, remaining, &frame->data_blocked.max_data, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
						 ...
						 struct tquic_frame *frame)
{
	...
	/* Stream ID */
	ret = tquic_varint_decode(p, remaining, &frame->stream_data_blocked.stream_id, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
	...
	ret = tquic_varint_decode(p, remaining, &frame->stream_data_blocked.max_stream_data, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
					     ...
					     struct tquic_frame *frame, bool bidi)
{
	...
	ret = tquic_varint_decode(p, remaining, &frame->streams_blocked.max_streams, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
					       ...
					       struct tquic_frame *frame)
{
	...
	/* Sequence Number */
	ret = tquic_varint_decode(p, remaining, &frame->new_cid.seq_num, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
	...
	/* Retire Prior To */
	ret = tquic_varint_decode(p, remaining, &frame->new_cid.retire_prior_to, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
						  ...
						  struct tquic_frame *frame)
{
	...
	/* Sequence Number */
	ret = tquic_varint_decode(p, remaining, &frame->retire_cid.seq_num, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
					      ...
					      bool app_close)
{
	...
	/* Error Code */
	ret = tquic_varint_decode(p, remaining, &frame->conn_close.error_code, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
	...
	if (!app_close) {
		ret = tquic_varint_decode(p, remaining, &frame->conn_close.frame_type, &consumed);
		if (ret < 0)
			return ret;
		p += consumed;
		remaining -= consumed;
	...
	ret = tquic_varint_decode(p, remaining, &frame->conn_close.reason_len, &consumed);
	if (ret < 0)
		return ret;
	p += consumed;
	remaining -= consumed;
			    ...
			    bool has_ecn, u64 ect0, u64 ect1, u64 ecn_ce)
{
	...
	size += tquic_varint_len(largest_ack);
	size += tquic_varint_len(ack_delay);
	size += tquic_varint_len(range_count);
	size += tquic_varint_len(first_ack_range);

	for (i = 0; i < range_count; i++) {
		size += tquic_varint_len(ranges[i].gap);
		size += tquic_varint_len(ranges[i].ack_range_len);
	}

	if (has_ecn) {
		size += tquic_varint_len(ect0);
		size += tquic_varint_len(ect1);
		size += tquic_varint_len(ecn_ce);
	}
...
{
	return 1 + tquic_varint_len(stream_id) +
	       tquic_varint_len(error_code) +
	       tquic_varint_len(final_size);
}
...
{
	return 1 + tquic_varint_len(stream_id) + tquic_varint_len(error_code);
}
...
{
	return 1 + tquic_varint_len(offset) + tquic_varint_len(length) + length;
}
...
{
	return 1 + tquic_varint_len(token_len) + token_len;
}
			       ...
			       bool has_offset, bool has_length)
{
	...
	size += tquic_varint_len(stream_id);

	if (has_offset)
		size += tquic_varint_len(offset);

	if (has_length)
		size += tquic_varint_len(length);
...
{
	return 1 + tquic_varint_len(max_data);
}
...
{
	return 1 + tquic_varint_len(stream_id) + tquic_varint_len(max_stream_data);
}
...
{
	return 1 + tquic_varint_len(max_streams);
}
...
size_t tquic_new_connection_id_frame_size(u64 seq_num, u64 retire_prior_to,
					  u8 cid_len)
{
	return 1 + tquic_varint_len(seq_num) + tquic_varint_len(retire_prior_to) +
...
{
	return 1 + tquic_varint_len(seq_num);
}
					 ...
					 u64 reason_len, bool app_close)
{
	...
	size += tquic_varint_len(error_code);

	if (!app_close)
		size += tquic_varint_len(frame_type);

	size += tquic_varint_len(reason_len);
	size += reason_len;
			  ...
			  bool has_ecn, u64 ect0, u64 ect1, u64 ecn_ce)
{
	...
	*p++ = has_ecn ? TQUIC_FRAME_ACK_ECN : TQUIC_FRAME_ACK;
	...
	/* Largest Acknowledged */
	ret = tquic_varint_encode(largest_ack, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;
	...
	/* ACK Delay */
	ret = tquic_varint_encode(ack_delay, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;
	...
	/* ACK Range Count */
	ret = tquic_varint_encode(range_count, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;
	...
	/* First ACK Range */
	ret = tquic_varint_encode(first_ack_range, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;
	...
	/* Additional ACK Ranges */
	for (i = 0; i < range_count; i++) {
		ret = tquic_varint_encode(ranges[i].gap, p, remaining);
		if (ret < 0)
			return ret;
		p += ret;
		remaining -= ret;

		ret = tquic_varint_encode(ranges[i].ack_range_len, p, remaining);
		if (ret < 0)
			return ret;
		p += ret;
		remaining -= ret;
	...
	/* ECN Counts (if present) */
	if (has_ecn) {
		ret = tquic_varint_encode(ect0, p, remaining);
		if (ret < 0)
			return ret;
		p += ret;
		remaining -= ret;

		ret = tquic_varint_encode(ect1, p, remaining);
		if (ret < 0)
			return ret;
		p += ret;
		remaining -= ret;

		ret = tquic_varint_encode(ecn_ce, p, remaining);
		if (ret < 0)
			return ret;
		p += ret;
		remaining -= ret;
				   ...
				   u64 error_code, u64 final_size)
{
	...
	*p++ = TQUIC_FRAME_RESET_STREAM;
	...
	ret = tquic_varint_encode(stream_id, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;

	ret = tquic_varint_encode(error_code, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;

	ret = tquic_varint_encode(final_size, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;
			     ...
			     const u8 *data, u64 data_len)
{
	...
	*p++ = TQUIC_FRAME_CRYPTO;
	...
	ret = tquic_varint_encode(offset, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;
				...
				const u8 *token, u64 token_len)
{
	...
	*p++ = TQUIC_FRAME_NEW_TOKEN;
	...
	ret = tquic_varint_encode(token_len, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;
...
int tquic_write_max_data_frame(u8 *buf, size_t buf_len, u64 max_data)
{
	...
	*p++ = TQUIC_FRAME_MAX_DATA;
	...
	ret = tquic_varint_encode(max_data, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;
				      ...
				      u64 max_stream_data)
{
	...
	*p++ = TQUIC_FRAME_MAX_STREAM_DATA;
	...
	ret = tquic_varint_encode(max_stream_data, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;
				  ...
				  bool bidi)
{
	...
	*p++ = bidi ? TQUIC_FRAME_MAX_STREAMS_BIDI : TQUIC_FRAME_MAX_STREAMS_UNI;
	...
	ret = tquic_varint_encode(max_streams, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;
					...
					u8 cid_len, const u8 *stateless_reset_token)
{
	...
	*p++ = TQUIC_FRAME_NEW_CONNECTION_ID;
	...
	ret = tquic_varint_encode(seq_num, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;

	ret = tquic_varint_encode(retire_prior_to, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;
				       ...
				       u64 reason_len, bool app_close)
{
	...
	*p++ = app_close ? TQUIC_FRAME_CONNECTION_CLOSE_APP :
	...
	if (!app_close) {
		ret = tquic_varint_encode(frame_type, p, remaining);
		if (ret < 0)
			return ret;
		p += ret;
		remaining -= ret;
	...
	ret = tquic_varint_encode(reason_len, p, remaining);
	if (ret < 0)
		return ret;
	p += ret;
	remaining -= ret;
...
{
	return tquic_varint_len(val);
}
EXPORT_SYMBOL_GPL(tquic_varint_encode_len);

/**
 * tquic_varint_read - Read a varint from a buffer
 * @buf: Input buffer
...
{
	return tquic_varint_decode(buf, buf_len, val, consumed);
}
EXPORT_SYMBOL_GPL(tquic_varint_read);

/**
 * tquic_varint_write - Write a varint to a buffer
 * @val: Value to write
 * @buf: Output buffer
 * @buf_len: Buffer length
 *
...
{
	return tquic_varint_encode(val, buf, buf_len);
}
