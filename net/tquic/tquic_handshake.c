...
static int tquic_inline_hs_install_keys(struct sock *sk, int level)
{
	...
	if (!crypto) {
		 ...
		 * Server: use original_dcid (client's random DCID).
		 * Client: use dcid (our random DCID for the server).
		 */
		const struct tquic_cid *init_cid = conn->is_server ?
			&conn->original_dcid : &conn->dcid;
		crypto = tquic_crypto_init_versioned(init_cid,
						     conn->is_server,
						     conn->version);
		if (!crypto) {
			ret = -ENOMEM;
				...
				int enc_level)
{
	...
	/*
	 * Process all TLS handshake records in the buffer.
	 * A single CRYPTO frame can contain multiple concatenated TLS
	 * messages (e.g. EE + Certificate + CertificateVerify + Finished).
	 * Each TLS message has a 4-byte header: type(1) + length(3).
	 */
	{
		u32 offset = 0;
		...
		while (offset < len) {
			...
			/* Parse TLS message: type(1) + length(3) + body */
			record_len = 4 +
			...
			ret = tquic_hs_process_record(ihs,
						      ...
						      &this_resp_len);
			if (ret < 0) {
				tquic_dbg("inline hs process_record failed at offset %u: %d\n",
					  offset, ret);
				kfree(resp_buf);
				tquic_inline_hs_abort(sk, ret);
				tquic_conn_put(conn);
					 ...
					 struct sk_buff *initial_pkt)
{
	 ...
	 *
	 * This is the random DCID the client generated before knowing the
	 * server's real CID.  Store it as original_dcid for:
	 *   - Initial key derivation (RFC 9001 Section 5.2)
	 *   - Transport parameter validation (RFC 9000 Section 7.3)
	 *
	 * The server keeps its own SCID (generated by tquic_conn_create).
	 */
	 ...
	 *   - DCID -> stored in conn->original_dcid (for Initial keys)
	 *   - SCID -> stored in conn->dcid (what we use to reach client)
	 *   - Token length (and skipped token data)
	 *   - Payload length
	 *
	 * The server keeps its own SCID (generated by tquic_conn_create).
	 * The client's original DCID is used only for Initial key derivation
	 * (RFC 9001 Section 5.2) and transport parameter validation.
	 *
	 * Per RFC 9000 Section 7.2: The client will update its DCID to our
	 * SCID from the first Initial response we send.
	 */
